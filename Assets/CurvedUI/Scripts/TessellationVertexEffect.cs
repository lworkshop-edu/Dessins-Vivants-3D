...
{
    [SerializeField]
    private float m_tessellationSize = 10.0f;
    ...
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        // Retrieve vertices from VertexHelper
        List<UIVertex> verts = new List<UIVertex>();
        vh.GetUIVertexStream(verts);

        // Assume we are working with quads
        if (verts.Count % 4 != 0)
        {
        ...
        // Remove the original vertices to avoid duplication
        verts.RemoveRange(0, startingVertexCount);

        // Apply modified vertices back to VertexHelper
        vh.Clear();
        vh.AddUIVertexTriangleStream(verts);
    }

    private void TessellateQuad(List<UIVertex> verts, int vertexIndex)
    {
    ...
    private UIVertex Bilerp(UIVertex v0, UIVertex v1, UIVertex v2, UIVertex v3, float a, float b)
    {
        UIVertex output = new UIVertex
        {
            position = Bilerp(v0.position, v1.position, v2.position, v3.position, a, b),
            normal = Bilerp(v0.normal, v1.normal, v2.normal, v3.normal, a, b),
            tangent = Bilerp(v0.tangent, v1.tangent, v2.tangent, v3.tangent, a, b),
            uv0 = Bilerp(v0.uv0, v1.uv0, v2.uv0, v3.uv0, a, b),
            uv1 = Bilerp(v0.uv1, v1.uv1, v2.uv1, v3.uv1, a, b),
            color = Bilerp(v0.color, v1.color, v2.color, v3.color, a, b)
        };
        return output;
    }

    private Vector3 Bilerp(Vector3 v0, Vector3 v1, Vector3 v2, Vector3 v3, float a, float b)
    {
    ...
    private Vector4 Bilerp(Vector4 v0, Vector4 v1, Vector4 v2, Vector4 v3, float a, float b)
    {
    ...
    private Color Bilerp(Color v0, Color v1, Color v2, Color v3, float a, float b)
    {
